...
def evaluate_model(
    ...
    return accuracy, f1_mes, precision, recall
...
def runOnDataset(file_name_without_extension: str):
    ...
    minorityTrainData = X_train[y == minority]
    ...
    SG_accuracy, SG_f1_score, SG_precision, SG_recall, model_SG = model_rf(X_SG, y_SG)
    ...
    G_accuracy, G_f1_score, G_precision, G_recall, model_G = model_rf(X_G, y_G)
    ...
    if file_name_without_extension == "drd":
        output_df = pd.DataFrame(
            [
                {
                    "Technique": "Validation Result (RF)",
    ...
    (
        ...
        Normal_test_f1_score,
        Normal_test_precision,
        Normal_test_recall,
    ) = evaluate_model(model_normal, X_test, y_test)
    (
        Smote_test_accuracy,
        Smote_test_f1_score,
    ...
    SG_test_accuracy, SG_test_f1_score, SG_test_precision, SG_test_recall = evaluate_model(
        model_SG, X_test, y_test
    )
    G_test_accuracy, G_test_f1_score, G_test_precision, G_test_recall = evaluate_model(
        model_G, X_test, y_test
    )

    if file_name_without_extension != "drd":
        ADA_test_accuracy, ADA_test_f1_score, ADA_test_precision, ADA_test_recall = (
            evaluate_model(model_ADA, X_test, y_test)
        )
        AG_test_accuracy, AG_test_f1_score, AG_test_precision, AG_test_recall = (
            evaluate_model(model_AG, X_test, y_test)
        )

    test_row = output_df.iloc[0].to_dict()
    test_row.update(
        {
            "Technique": "Testing Result (RF)",
            "Normal_accuracy": Normal_test_accuracy,
            "Normal_f1_score": Normal_test_f1_score,
            "Normal_precision": Normal_test_precision,
            "Normal_recall": Normal_test_recall,
            "SMOTE_accuracy": Smote_test_accuracy,
            "SMOTE_f1_score": Smote_test_f1_score,
            "SMOTE_precision": Smote_test_precision,
            "SMOTE_recall": Smote_test_recall,
            "SG_accuracy": SG_test_accuracy,
            "SG_f1_score": SG_test_f1_score,
            "SG_precision": SG_test_precision,
            "SG_recall": SG_test_recall,
            "G_accuracy": G_test_accuracy,
            "G_f1_score": G_test_f1_score,
            "G_precision": G_test_precision,
            "G_recall": G_test_recall,
        }
    )
    ...
    if file_name_without_extension != "drd":
        test_row.update(
            {
                "ADASYN_accuracy": ADA_test_accuracy,
                "ADASYN_f1_score": ADA_test_f1_score,
                "ADASYN_precision": ADA_test_precision,
                "ADASYN_recall": ADA_test_recall,
                "AG_accuracy": AG_test_accuracy,
                "AG_f1_score": AG_test_f1_score,
                "AG_precision": AG_test_precision,
                "AG_recall": AG_test_recall,
            }
        )

    output_df = pd.concat(
        [output_df, pd.DataFrame([test_row], columns=output_df.columns)],
        ignore_index=True,
    )

    output_df.to_csv(
        f"NewResults/{file_name_without_extension}_result.csv", index=False
    )
